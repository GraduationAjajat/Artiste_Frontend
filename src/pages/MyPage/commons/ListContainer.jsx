...
const ListContainer = ({ contents, type }) => {
  ...
  const clickApproval = (id) => {
    axios.put(`/api/v1/admin/waiting/${id}`).then((res) => {
      setIsApproval(true);
    });
  };
          ...
          <>
            <ContentContainer>
              ...
              <Content>
                  ...
                  <div>
                    {type === "gallery" ? (
                      content.exhibitionState === "APPROVAL" ? (
                        <ApprovalBtn>승인됨</ApprovalBtn>
                      ) : (
                        <ApprovalBtn color="black">대기중</ApprovalBtn>
                      )
                    ) : (
                      <></>
                    )}
                    <BlackText weight={"700"} size={"20px"}>
                ...
                <RightSection>
                  <BlackText weight={"500"} size={"15px"}>
                    {content.exhibitionStartDate.substring(0, 10)}
                  </BlackText>
                  {type === "admin" ? (
                    <ApprovalBtn
                      style={{ margin: "10px 0 0 0" }}
                      onClick={() => clickApproval(content.exhibitionId)}
                      color={isApproval === true ? "black" : ""}
                    >
                      승인
                    </ApprovalBtn>
                  ) : (
                    <RowContainer>
                      <img src="../../imgs/heart.svg" alt="heart"></img>
                      <Num>{content.scrapCount}</Num>
                      <img src="../../imgs/comment.svg" alt="comment"></img>
