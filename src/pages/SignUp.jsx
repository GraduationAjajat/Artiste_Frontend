...
const SignUp = () => {
  ...
  const clickCheckEmailBtn = () => {
    console.log(email);
    axios.get(`/api/v1/user/check/email/${email}`).then((res) => {
      if (res.data === false) {
  ...
  const clickCheckNicknameBtn = () => {
    axios.get(`/api/v1/user/check/nickname/${nickname}`).then((res) => {
      if (res.data === false) {
  ...
  return (
    <SignUpContainer>
      ...
      <Content>
        ...
        <GrayText marginTop={"4px"}>
          아르티스트를 이용하시려면 회원가입을 진행해주세요
        </GrayText>
        <Form>
          ...
          <Label>
            ...
            <CheckEmailBtn type="button" onClick={() => clickCheckEmailBtn()}>
              중복확인
            </CheckEmailBtn>
          </Label>
          ...
          <GrayRoundInput
            onChange={(e) => onChangePwConfirm(e)}
            type={"password"}
          />
          ...
          <RowContainer style={{ margin: "15px 0 20px 0" }}>
            {sex === 0 ? (
              <img src="../imgs/checked.svg" onClick={handleClick}></img>
            ) : (
              <img src="../imgs/unchecked.svg" onClick={handleClick}></img>
            )}
